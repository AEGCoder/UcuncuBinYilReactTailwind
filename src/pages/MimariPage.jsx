...
const MimariPage = () => {
  ...
  const debouncedFilterByTitle = debounce(async (title) => {
    try {
      const response = await axios.get("/MimariData.json");
  ...
  const items = [
    {
      key: '1',
      label: 'Mimarlık ve İç Mimar  İş İlanları',
      children: <p>Üçüncü Binyıl öğrencileri işe daha hızlı yerleşiyor. Hem aldığı eğitimin kalitesi, hem de iş yönlendirme sayesinde, öğrencilerimiz işe daha hızlı yerleşiyor. Mimar ve İç Mimar iş ilanları için TeknikKariyer</p>,
    },
    {
      key: '2',
      label: 'Mimarlık ve İç Mimarlıkta Kullanılan Programlar',
      children: <p>Metraj Hakediş ve Şantiye Yapı İşletmesi, Rhinoceros Architecture, Rhinoceros Grasshopper, Revit Architecture, Revit Dynamo, Revit Structure, 3Ds MAX, Sketch UP, Photoshop Post Production, AutoCAD, Lumion 3D, Revit Mep, Unreal Engine, V-Ray, ArchiCAD</p>,
    },
    {
      key: '3',
      label: 'Sertifikalı Eğitimler',
      children: <p>Eğitimlerimizde Uluslararası Geçerliliğe Sahip Sertifikalar Vermekteyiz. Sektörde en çok bilinen eğitim kurumu Üçüncü Binyıldır. TMMOB Mimarlar Odası kaydı TMMOB İç Mimarlar Odası kaydı</p>,
    },
  ];
  ...
  return (
    <div className="w-full pt-10">
      <div className="relative">
        <video
          src="./videos/mimariVideo.mp4"
          className=" h-full w-full rounded-sm"
          autoPlay
          loop
          muted
        ></video>
        <span className=" absolute left-10 sm:bottom-10 bottom-0 sm:text-4xl text-2xl text-black font-bold tracking-wider">
          MİMARİ EĞİTİMLERİ
        </span>
      </div>
      <div>
        <ul className="flex items-center sm:gap-x-5 gap-x-2 text-sm sm:p-10 p-3 border-b border-gray-400">
          <li>
            <a href="">Anasayfa</a>
          </li>
          <li>
          ...
          <li>
            <a href="">Mimari Eğitimleri</a>
          </li>
        </ul>
      </div>

      <div className="flex sm:flex-row flex-col sm:gap-y-0 gap-y-10 items-start sm:gap-x-5 gap-x-2 sm:p-10 p-2">
        {/* -----------------left component------------------- */}
          ...
          <>
            <ul className="grid sm:grid-cols-3 grid-cols-1 gap-y-10 pl-8 sm:pl-0">
              {currentItems.map((kurs) => (
                <Link key={kurs.id} to={`/mimari/${kurs.titleParams}`}>
                <li key={kurs.id}>
