...
import MeslekiBilisimAciklama from "../../components/MeslekiBilisimAciklama";
...
const MeslekiBilisimState = () => {
  const [state, setstate] = useState(0);
  const [meslekiBilisimData, setMeslekiBilisimData] = useState([]);
  const { titleParams } = useParams();

  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get("/MeslekiBilisimData.json");
        const data = response.data;
        const MeslekiBilisimGet = data.find(
          (mesleki) => mesleki.titleParams === titleParams
        );
        setMeslekiBilisimData(MeslekiBilisimGet);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    ...
    <div className="">
      <ul className="container  mx-auto flex items-center  sm:gap-x-5 gap-x-2 sm:text-sm text-xs sm:p-10 p-3  border-b border-gray-400">
        ...
        <li>
          <a href="">Mesleki Bilişim Eğitimleri</a>
        </li>
        <li>
          <a href="">{meslekiBilisimData && meslekiBilisimData.title}</a>
        </li>
      </ul>
        ...
        <div className="sm:w-[70%] w-full flex flex-col gap-y-10">
          <div>
            <h1 className="font-bold sm:text-3xl text-xl text-black">
              {meslekiBilisimData && meslekiBilisimData.title}
            </h1>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex flex-col text-sm">
              <span className="text-gray-500">Kategoriler</span>
              <span>MESLEKİ BİLİŞİM EĞİTİMLERİ</span>
            </div>
                ...
                <ul className="flex flex-col gap-y-7 font-extralight text-sm">
                  <li className="flex items-center gap-x-10 border-b pb-4">
                    ...
                    <div className="flex items-center gap-x-2">
                      <span>{meslekiBilisimData && meslekiBilisimData.DersSaati}</span>
                    </div>
                  </li>
                  <li className="flex items-center gap-x-10 border-b pb-4">
                    <div className="flex items-center gap-x-2  w-40">
                    ...
                    <div className="flex items-center gap-x-2">
                      <span>{meslekiBilisimData && meslekiBilisimData.KursSuresi}</span>
                    </div>
                  </li>
...
export default MeslekiBilisimState;
