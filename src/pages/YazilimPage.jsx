...
const YazilimPage = () => {
  ...
  const debouncedFilterByTitle = debounce(async (title) => {
    try {
      const response = await axios.get("/YazilimData.json");
  ...
  return (
    ...
    className="w-full pt-10">
      <div className="relative">
        <video
          src="./videos/YazilimVideo.mp4"
          className=" h-full w-full rounded-sm"
          autoPlay
          loop
          muted
        ></video>
        <span className=" absolute left-10 sm:bottom-10 bottom-1 sm:text-4xl text-xl text-white font-bold tracking-wider">
          YAZILIM EĞİTİMLERİ
        </span>
      </div>
      <div>
        <ul className="flex items-center sm:gap-x-5 gap-x-2 text-sm sm:p-10 p-5 border-b border-gray-400">
          <li>
            <a href="">Anasayfa</a>
          </li>
          <li>
          ...
          <li>
            <a href="">Yazılım Eğitimleri</a>
          </li>
        </ul>
      </div>

      <div className="flex sm:flex-row flex-col sm:gap-y-0 gap-y-10 items-start gap-x-5 sm:p-10 p-2">
        {/* -----------------left component------------------- */}
          ...
          <>
            <ul className="grid sm:grid-cols-3 grid-cols-1 gap-y-10 pl-8 sm:pl-0">
              {currentItems.map((kurs) => (
                <Link key={kurs.id} to={`/yazilim/${kurs.titleParams}`}>
                <li key={kurs.id}>
                  <div className="border-2 relative shadow-sm p-2 h-[360px] w-[290px]">
                    ...
                    <div className="w-full h-[50%] flex flex-col items-center justify-center">
                      <div>
                        <span className="text-xs">{kurs.minTitle} </span>
                      </div>
